    ...
    def train(self, dataset, labels, epochs, batch_size, path_gt_pred='gt_pred'):
        ...
        for i in range(1, epochs):
            ...
            for j in range(nbbatches):
                batch_x_vec, batch_s = next(train_batch_gen)
                b_x = batch_x_vec
                b_y = batch_s
            ...
            if i % 100 == 0:
                msg = "\tEpoch %d/%d Loss %0.4f Adversarial task %s train Acc %0.4f test Acc %0.4f" % \
                      (i, epochs, epoch_loss, self.task, epoch_acc, test_acc)
                print(msg)
...
if __name__ == "__main__":
    ...
    if task == 's1' or task == 'S1':
        label, num_class = load_labels(path_s1)
    else:
        label, num_class = load_labels(path_s2)
    target = make_onehot(label, num_class)
    adversary_model = Adversary(num_class)
    adversary_model.task = task
    adversary_model.train(vec, target, epochs=1000, batch_size=SPLIT_INDEX, path_gt_pred=path_gt_pred)
